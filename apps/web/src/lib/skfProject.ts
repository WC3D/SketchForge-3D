...
async function restoreShapeFromNode(
  ...
  const sculptSource = node.sculptSourceNodeId
    ? await restoreShapeFromNode(node.sculptSourceNodeId, nodeById, assetById, files, runtimeAssetByArchiveId, sourceMeshCache, derivedMeshCache, sourceImporter, resources, new Set(restoring))
    : undefined;
